<?php
    ...
    function get_user()
    {
        $rows = CS50::query("SELECT username FROM `final project` . `users` WHERE id = ?", $_SESSION["id"]);
    ...
    {
        $rows = CS50::query("SELECT * FROM `final project`. `scores` WHERE username = ? ORDER BY battle_index DESC LIMIT 1", $username);
